...
\begin{algorithm}[H]
  ...
  nb\_arcs $\leftarrow$ nbr\_vertices*rate\;
  
  \textit{//génération d'un chemin}\\
  \Pour{$u = 1$; $u < $nbr\_vertices; $++u$ }{
  ...
  \Tq{nbr\_arcs$ < 0$}{
    $u \leftarrow$ rand \% nb\_vertices\;
    $v \leftarrow$ rand \% nb\_vertices\;
    
    \Si{$u \neq $ nbr\_vertices$ - 1$ \textbf{ET} $v \neq 0$ \textbf{ET} !G.contains($u,v$)}{
      weight $\leftarrow$ rand(min\_weight, max\_weight)\;
      G.add($u$,$v$,weight)\;
      --nbr\_arcs\;
    }
