\subsection{Fonction chromatique}
...
\subsection{Des cas un peu spéciaux\ldots}
$\chi(G)$ étant, par définition, le nombre minimum de couleurs nécessaires pour colorier $G$, si $k < \chi(G)$ alors le graphe $G$ ne peut pas être colorié par $k$ couleurs. Si $k \geq \chi(G)$ alors il doit y avoir au moins une manière de colorier $G$, celui utilisant $\chi(G)$ couleurs.
...
\subsection{Application à $K_{1,5}$}
...
\subsection{Application aux graphes non connexes}
...
\subsection{Application aux arbres}
arbre

\subsection{$k^5 - 4k^4 + 6k^3 - 4k^2 + k$}
D'après la propriété de la question $6$, nos trois graphes $G_1$, $G_2$ et $G_3$ auront 5 sommets et 4 arrêtes chacun.   

\subsection{Application à $K_{2,5}$}
polychrome of K2,5

\subsection{Application aux cycles}
\begin {itemize}
...
\subsection{Démonstration sur les cycles}
cycles générales

\subsection{Démonstration sur les graphes bipartis complets}
