...
\[ \mathcal{C}_k = (l_1 \vee l_2 \vee v_1) \bigwedge_{i=1}^{k-4}(\neg v_i \vee l_{i+2} \vee v_{i+1}) \wedge (\neg v_{k-3} \vee l_{k-1} \vee l_{k}) \]
...
\begin{description}
 ...
 \item \textbf{3--SAT $\rightarrow$ SAT}
 	...
 	\begin{description}
		\item[Cas 1] $val(I_k,(l_1 \vee l_2 \vee v_1)) = vrai$
		\item[Cas 2] $val(I_k,(v_{k-3} \vee l_{k-1} \vee l_k)) = vrai$
 	\end{description}
